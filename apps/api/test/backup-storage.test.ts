...
describe("private Supabase backup storage", () => {
  it("accepts only an existing private bucket", async () => {
    ...
    await expect(new SupabaseBackupStorage("https://example.supabase.co", "service-secret", "maktaba-backups").assertPrivateBucket()).resolves.toBeUndefined();
  ...
  it("rejects a public or missing bucket", async () => {
    ...
    await expect(new SupabaseBackupStorage("https://example.supabase.co", "key", "maktaba-backups").assertPrivateBucket()).rejects.toThrow("privé");
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response("not found", { status: 404 })));
    await expect(new SupabaseBackupStorage("https://example.supabase.co", "key", "maktaba-backups").assertPrivateBucket()).rejects.toThrow("404");
  });
  it("propagates upload failures and forbids overwrite", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    await expect(new SupabaseBackupStorage("https://example.supabase.co", "key", "maktaba-backups").upload("daily/a.dump", Buffer.from("data"), "application/octet-stream")).rejects.toThrow("503");
